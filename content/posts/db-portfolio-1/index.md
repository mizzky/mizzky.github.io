...
- ゴール: _SQLを投げたときに、DBが全件読んでいるのか、インデックスを使っているのかを実行計画から読める。_
  - Seq Scan / Index Scan / Bitmap Scan / Sort の意味を理解し、B-Treeインデックスや複合インデックスがどの条件で効いているかを説明できる。
...
- ゴール: _データ件数が増えたときに、SQLやAPI設計のどこがボトルネックになりやすいかを説明できる。_
  - LIMIT/OFFSET、keyset pagination、N+1、JOIN / batch取得の違いを理解し、このECアプリのどのAPIにページングや取得方法の見直しが必要になりそうかを整理できる。
...
- ゴール: _Goの `sql.DB` が接続プールであることを理解し、大量リクエスト時にアプリ側の接続設定がDB負荷や待ち時間にどう影響するかを説明できる。_
  - `SetMaxOpenConns`, `SetMaxIdleConns`, `SetConnMaxLifetime` の役割を理解し、このアプリで設定する場合の判断基準を整理できる。

これらの知識をつけて理論面の学習ができた段階で、リポジトリ内のDBを使って手を動かすことで理解を定着させることを狙う。
